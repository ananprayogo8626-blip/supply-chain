<x-app-layout>
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between pb-5 mb-6 border-b border-white/5">
        <div>
            ...
            <p class="text-sm text-slate-400 mt-2">Monitor integration status and manually trigger GNews API background synchronization.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="api-card">
            <div class="flex items-center justify-between mb-4">
                <span class="text-xs text-slate-400 font-semibold uppercase">API Status</span>
            ...
            <p class="text-[11px] text-slate-400 mt-4">GNews integration health status</p>
        </div>

        <!-- Last Sync -->
        <div class="api-card">
            <div class="flex items-center justify-between mb-4">
                <span class="text-xs text-slate-400 font-semibold uppercase">Last Sync</span>
                <i data-lucide="clock" class="w-5 h-5 text-blue-400"></i>
            </div>
            <h3 class="text-xl font-bold text-white mt-2">{{ $lastSync }}</h3>
            <p class="text-[11px] text-slate-400 mt-4">Time since latest successful pull</p>
        </div>

        <!-- Masked Key -->
        <div class="api-card">
            <div class="flex items-center justify-between mb-4">
                <span class="text-xs text-slate-400 font-semibold uppercase">API Key Configured</span>
                <i data-lucide="key" class="w-5 h-5 text-amber-400"></i>
            </div>
            <h3 class="text-sm font-mono text-slate-300 mt-3 truncate" title="{{ $maskedKey }}">
                {{ $maskedKey }}
            </h3>
            <p class="text-[11px] text-slate-400 mt-4">Loaded from services config</p>
        </div>

        <!-- Total Articles -->
        <div class="api-card">
            <div class="flex items-center justify-between mb-4">
                <span class="text-xs text-slate-400 font-semibold uppercase">Total Articles</span>
                <i data-lucide="newspaper" class="w-5 h-5 text-violet-400"></i>
            </div>
            <h3 class="text-2xl font-bold text-white mt-1">{{ number_format($totalArticles) }}</h3>
            <p class="text-[11px] text-slate-400 mt-4">Cached news records in database</p>
        </div>
    </div>
        ...
        <div class="sg-data-head">
            <div class="sg-data-head-left">
                ...
                <h2 class="sg-data-title">Synchronization Logs
                    <span class="sg-count-badge">GNews history</span>
                </h2>
            </div>
        </div>
                    ...
                    @forelse($syncLogs as $log)
                        <tr>
                            ...
                            <td class="font-semibold text-slate-300">
                                {{ $log->processed }} / {{ $log->total }} articles
                            </td>
                            <td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center text-slate-400 py-8">
                                No synchronization logs found. Click "Sync GNews Now" to run your first sync.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <script>
        function triggerSync() {
            Swal.fire({
                ...
                color: '#F8FAFC',
                didOpen: () => {
                    fetch('{{ route('admin.api-management.sync') }}', {
                        method: 'POST',
                        headers: {
        ...
        function pollSyncProgress() {
            const interval = setInterval(async () => {
                try {
                    const res = await fetch('/api/import/progress/news');
                    ...
                    if (statusText) {
                        statusText.textContent = `Progress: ${json.percentage}% | Fetched: ${json.processed}/${json.total} articles`;
                    }
                    ...
                    if (json.status === 'failed') {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Sync Failed',
                            text: 'Latest news synchronization failed. Please check the logs.',
                            confirmButtonColor: '#FF6B00',
                            background: '#1B2433',
                            color: '#F8FAFC'
                        });
